...
## Hybrid Search 방식: BM25(키워드) + Vector(의미) 결합
...
# ========================================
# BM25 인덱스 구축 (초기 로딩 시 한번만)
# ========================================

def build_bm25_index():
    """벡터DB에서 모든 문서를 가져와 BM25 인덱스 구축"""
    print("[BM25] 인덱싱 중...")
    ...
    print(f"[BM25] {len(documents)}개 문서로 인덱싱 완료")
    
    return bm25, documents, metadatas


bm25, all_documents, all_metadatas = build_bm25_index()
...
def hybrid_search(query: str, k: int = 10, bm25_weight: float = 0.5, vector_weight: float = 0.5):
    """
    ...
    Args:
        ...
        vector_weight: 벡터 유사도 가중치 (기본 0.5)
    
    Returns:
        상위 k개 문서 리스트 (LangChain Document 형식)
    """
    
    print(f"\n[Hybrid Search] 쿼리: '{query}'")
    print(f"  가중치 - BM25: {bm25_weight}, Vector: {vector_weight}")
    ...
    bm25_scores_normalized = bm25_scores / (np.max(bm25_scores) + 1e-10)
    
    print(f"  [BM25] 상위 점수: {sorted(bm25_scores_normalized, reverse=True)[:5]}")
    
    # 2. 벡터 유사도 검색
    vector_results = vectorstore.similarity_search_with_score(query, k=len(all_documents))
    
    # 벡터 유사도를 0-1 범위로 정규화 (distance를 similarity로 변환)
    vector_scores_dict = {}
    for doc, distance in vector_results:
        similarity = max(0, 1 - (distance / 2))  # distance -> similarity 변환
        # 문서 내용으로 인덱싱 (동일한 문서 찾기용)
        vector_scores_dict[doc.page_content] = similarity
    
    print(f"  [Vector] 상위 점수: {sorted(list(vector_scores_dict.values()), reverse=True)[:5]}")
    
    # 3. 하이브리드 점수 계산
    hybrid_scores = []
    
    for idx, (doc_content, bm25_score) in enumerate(zip(all_documents, bm25_scores_normalized)):
        vector_score = vector_scores_dict.get(doc_content, 0.0)
    ...
    # 4. 하이브리드 점수로 정렬 및 상위 k개 선택
    hybrid_scores.sort(key=lambda x: x["hybrid_score"], reverse=True)
    top_results = hybrid_scores[:k]
    
    print(f"  [Hybrid] 상위 점수: {[r['hybrid_score'] for r in top_results[:5]]}")
    print(f"  [결과] {len(top_results)}개 문서 선택됨")
    
    # 5. LangChain Document 형식으로 변환
...
if __name__ == "__main__":
    ...
    for i, q in enumerate(test_questions, 1):
        ...
        print(f"[QT 변환] {transformed_q}")

        # 프롬프트 선택 (원본 질문 기준)
        prompt = choose_prompt(q)

        # Hybrid Search로 문서 검색
        # 가중치 기본값: BM25 0.5, Vector 0.5 (균형잡힌 방식)
        docs = hybrid_search(transformed_q, k=10, bm25_weight=0.5, vector_weight=0.5)
