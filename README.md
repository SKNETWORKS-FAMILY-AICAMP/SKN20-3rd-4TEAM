...
| program_chunk | 700 | 100 | 이미 청킹된 파일 (기본값) |
| ip_manual_chunk | 700 | 100 | 이미 청킹된 파일 (기본값) |
...
> #### 김태빈
> LLM으로 RAG를 구현하여 동작시키는 것이 많은 깨달음을 얻게했습니다. 장차 커다란 힘을 얻어가는 기분이었습니다.
---
...
> #### 정래원
> 챗봇을 구현하는 과정 자체는 생각보다 수월하다고 느꼈지만, 프롬프트 구성이나 데이터 내용이 조금만 달라져도 답변 결과가 크게 달라져 세심한 설계의 중요성을 느꼈습니다!
그럼에도 불구하고 좋은 팀원들과 파트를 명확히 분담해 효율적으로 협업할 수 있었서 좋았습니다!
